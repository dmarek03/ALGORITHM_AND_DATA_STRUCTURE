...
def heavy_path(T):
    ...
    # In first_path and second_path we hold the longest paths
    ...
    if first_path + second_path > T.longest_path:
        # We check if actual path is longer than the longest path
        T.longest_path = first_path + second_path
    # At the end we return the longest path
    # Total complexity of this algorithm is O(n) where n is the number of nodes
    return first_path
